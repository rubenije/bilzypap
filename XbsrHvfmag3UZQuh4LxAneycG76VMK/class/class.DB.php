...
define( 'DB_NAME', 'ccu' ); // set database name
define( 'SEND_ERRORS_TO', 'ruben.ricarte@gmail.com' ); //set email notification email address
...
class DB
{
    ...
    public function init(){
        ...
        if($_SERVER['HTTP_HOST'] == 'localhost:8080'){
            ...
            $this->dbUser = 'bilzypap_prod_user';
            $this->dbPass = 'Qwerty12358.';
            $this->dbName = 'bilzypap_prod_db';
            $this->dbPort = 3306;
        }   
    }
    ...
    */
    ...
    public function uploadFileImage($name, $old_imagen = ''){  
        ...
        $folder = self::createDirectory();

        $handle = new \Verot\Upload\Upload($_FILES[$name]);
        if ($handle->uploaded) {
            if(!empty($old_imagen)){
                unlink($folder.$old_imagen);
            }
    ...
    public function uploadFileArchivoBases($name, $old_imagen){  
        ...
        if ($handle->uploaded) {
            ...
            $rand = date('YmdHis');
            $handle->file_new_name_body   = 'bases-legales-escudo-'.$rand;
            $handle->process('../uploads/');
            if ($handle->processed) {
                $upload = true;
                $handle->clean();
